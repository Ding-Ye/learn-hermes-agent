...
// Session — same shape as s04. Persistence stays JSON files for the
// session itself (this chapter's new SQLite db is for memories, not
// sessions). s09 unifies them on a multi-process backend.
...
func newSessionID(now time.Time) string {
	var rb [3]byte
	_, _ = rand.Read(rb[:])
	return fmt.Sprintf("s05-%s-%s", now.Format("20060102-150405"), hex.EncodeToString(rb[:]))
}
